// make proplist array
line = rawPropertyListStr.split('\n')
var U = []
for (let i=0;i<line.length;i++) {
	if (line[i].startsWith('#')) continue
	if (line[i] == '') continue
	spaceArray = line[i].split('#') // eject the comments
...
function makeList (stream) {
    // clear the legend
    var ps = document.querySelectorAll('#legend p')
	...
	for (x=0;x<keys.length;x++) {
		out += '<tr class="main">'
		out += '<th class="sg">'+keys[x].trim()+'</th>'
        if (document.getElementById(keys[x].trim()) === null) console.log('Missing legend:',keys[x])
        else document.getElementById(keys[x].trim()).style.display = 'block'
