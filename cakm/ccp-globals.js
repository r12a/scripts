

langTag='ccp' // BCP47 language tag for this orthography
scriptTag='cakm' // BCP47 script tag for this orthography
notesLangtag='ccp'
blockLangtag = 'ccp' // same as langTag; used in block.html
blockDirection=''  // ltr/rtl or '' for ltr; used by block.html

blockDirectoryName='cakm'  // folder name where block file is found, no slash
orthogFilePath='cakm/ccp'  // path below /scripts, including filename, but no extension, eg. lana/nod
pickerDir='cakm-ccp'  // directory name for picker, no slash at end
orthogSummaryTableName='ccp'  // id used for langs record in xx-langdata.js; used to make summary table
scriptSummaryTableName='cakm'  // id used for langs record in xx-langdata.js; used to make summary table
charUsageBCP='ccp'  // bcp tag to find the orthography in the charuse app; used in makeIndexIntro only
orthogName = 'Chakma' // natural language name for the orthography; used only in makeIndexIntro
languageName = 'Chakma' // natural language name of language; used in various places, esp block
scriptIsIndic = true  // true if there are indic syllabic categories; used only in makeIndexIntro


defaultSize = '24px'  // used by block.html to set size of font, including in font mgt panel
letterClassName = 'ccp'  // classname for a given orthography in the block page
hideBlockName = 'CHAKMA' // if a value is given, that text is removed from code point names