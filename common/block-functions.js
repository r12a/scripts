...
function initialise (base, ghDirectory, detailsfileList, version) {
    ...
    // charappDir, directory name for the character app
...
function addDetails (languageName, langClass, lang, dir, spreadsheet, cols) {
    ...
    charappDir, str, directory name for the character app
    charDetails, obj, the detailed notes, created in xx-examples.js
    */
    /* LOCAL
    ...
    for (var x=0; x<noteList.length; x++) {
		...
		if (spreadsheetRows[cchar]) {
           ...
           // add a link to the script notes
            if (window.explainerDir || window.charappDir) {
                ...
                if (window.explainerDir && window.charappDir) out += ' &bull; '

                // add a link to the character app
                if (window.charappDir) {
                    out += '<a target="_blank" href="/pickers/'+window.charappDir+'?text='+encodeURIComponent(cchar)+'">character app</a> '
                    }
