...
function showCharDetails (ch) {
    ...
    // keep the original ch for shape mgt, etc.
    CH = ch
   ...
   // div.innerHTML = `<a href="../${ orthogFilePath }.html?showIndex#index${ ch }" target="_blank">Find in orthography notes.</a> •
    ...
    // expand any .characterMarkup elements (mainly for cursive or subjoined items)
    ...
    if (cols['shape'] && cols['shape'] !== 0) {
        for (i=0;i<shapes.length;i++) {
            ...
            console.log('CH',CH)
            console.log(spreadsheetRows[CH] && spreadsheetRows[CH][cols['shape']] !== '')
