...
// get a list of characters for the xx-character files
    ...
    for (chr in spreadsheetRows) {
        ...
        if (typeof spreadsheetRows[chr][cols['status']] === 'undefined' ||
            ...
            //{ console.log('Avoiding',chr,'status is ',spreadsheetRows[chr][cols['status']])} // UNCOMMENT THIS
            { } // do nothing
        else out += chr
        }
...
makeAutoTranslitArray(blockLangtag)
addExamples(letterClassName)
...
function showCharDetails (ch) {
    // creates the output for a single character, called repeatedly by makeXXCharacterPage
    ...
    hex = ch.codePointAt(0).toString(16).toUpperCase()
    while (hex.length < 4) hex = '0' + hex
	dec = parseInt(hex, 16)
    cpName = spreadsheetRows[ch][cols['ucsName']]
   ...
   // check for decomposable characters
    if (ch.normalize('NFD') != ch) {
        out += '<p class="decomposition">'
        out += 'Decomposes to '+makeCharacterLink(ch.normalize('NFD'), lang, dir)
        if (ch.normalize('NFD') === ch.normalize('NFC')) out += '<br><strong>The NFC normalised form of this character is the decomposed sequence!</strong>'
        out += '</p>'
        }
    ...
    out += addDetails(ch, languageName, letterClassName, blockLangtag, blockDirection, spreadsheet, cols)
...
function addDetails (ch, languageName, langClass, lang, dir, spreadsheet, cols) {
    ...
    */
    ...
    var hex, dec // alternatives for ch
    ...
    // Add the current character - use shape column, if it is defined
    if (spreadsheetRows[ch] && charDetails[ch]) {
        var out = '<div class="letter '+langClass+'" id="'+langClass+hex+'">'

        out += `<div lang="${ lang }" class="currentCharacter">`

        if (cols['shape'] !== 0 && spreadsheetRows[ch] && spreadsheetRows[ch][cols['shape']] && spreadsheetRows[ch][cols['shape']] !== '') {
            if (spreadsheetRows[ch][cols['shape']] === '4') out += `${ ch } ${ ch }${ ch }${ ch }`
            else if (spreadsheetRows[ch][cols['shape']] === '2') out += `${ ch } \u200D${ ch }`
            else out += spreadsheetRows[ch][cols['shape']]
            }
        else out += ch
        ...
        out += `</div>`
			...
			//if (spreadsheetRows[ch][cols['meaning']] && spreadsheetRows[ch][cols['meaning']] != '0') out += '<span class="charMeaning meaning">'+spreadsheetRows[ch][cols['meaning']]+'</span>'
			
			//out += '</p>'

            out += `<p class="basicInfo">`
			if (spreadsheetRows[ch][cols['typeLoc']]) out += '<span class="charType" title="Type of character.">'+spreadsheetRows[ch][cols['typeLoc']]+'</span>'
			
			if (spreadsheetRows[ch][cols['statusLoc']]) out += ' <span class="charType" title="Usage information.">('+spreadsheetRows[ch][cols['statusLoc']]+')</span>'
			
			if (spreadsheetRows[ch][cols['status']]) out += '<span class="charStatus" title="Status information.">'+statusExpander(spreadsheetRows[ch][cols['status']])+'</span>'
			
			//if (spreadsheetRows[ch][cols['ipaLoc']]) out += '<span class="charIPA ipa" title="Typical IPA phonetic values.">'+spreadsheetRows[ch][cols['ipaLoc']].toLowerCase()+'</span>'
            
			if (spreadsheetRows[ch][cols['nameLoc']] && spreadsheetRows[ch][cols['nameLoc']] != '0') out += '<span class="transliteratedname trans">'+spreadsheetRows[ch][cols['nameLoc']]+'</span>'
			
			if (spreadsheetRows[ch][cols['nnameLoc']] && spreadsheetRows[ch][cols['nnameLoc']] != '0') out += '<span class="localname" lang="'+lang+'">'+spreadsheetRows[ch][cols['nnameLoc']]+'</span>'
			
			
			//if (spreadsheetRows[ch][cols['transLoc']]) out += '<span class="localtrans trans" title="How transliterated in the scheme used for this document.">'+spreadsheetRows[ch][cols['transLoc']]+'</span>'
			
			if (spreadsheetRows[ch][cols['transLoc']]) out += '<span class="transLoc trans" title="How transliterated in the scheme used for this document.">'+spreadsheetRows[ch][cols['transLoc']]+'</span>'
			
			if (spreadsheetRows[ch][cols['class']]) out += '&nbsp; <span class="class" title="Unicode General Category property value.">'+spreadsheetRows[ch][cols['class']]+'</span>'

            out += '</p>'
           
            
			// vowel correspondences
			if (cols.ivowel>0 && spreadsheetRows[ch][cols.ivowel]) {
				out += '<p class="vowelPairing">The corresponding independent vowel is '+makeCharacterLink(spreadsheetRows[ch][cols.ivowel], lang, dir)+'</p>'
				}
			if (cols.dvowel>0 && spreadsheetRows[ch][cols.dvowel]) {
				out += '<p class="vowelPairing">The corresponding dependent vowel is '+makeCharacterLink(spreadsheetRows[ch][cols.dvowel], lang, dir)+'</p>'
				}

			// upper/lowercase
			if (cols.uc>0 && spreadsheetRows[ch][cols.uc]) {
				out += '<p class="charUppercase">Uppercase is '+makeCharacterLink(spreadsheetRows[ch][cols.uc], lang, dir)+'</p>'
				}
			if (cols.lc>0 && spreadsheetRows[ch][cols.lc]) {
				out += '<p class="charLowercase">Lowercase is '+makeCharacterLink(spreadsheetRows[ch][cols.lc], lang, dir)+'</p>'
				}

			// subjoined forms
			if (cols.subj>0 && spreadsheetRows[ch][cols.subj]) {
				out += '<p class="subjPair">Subjoined form is '+makeCharacterLink(spreadsheetRows[ch][cols.subj], lang, dir)+'</p>'
				}
			if (cols.fform>0 && spreadsheetRows[ch][cols.fform]) {
				out += '<p class="subjPair">Non-subjoined form is '+makeCharacterLink(spreadsheetRows[ch][cols.fform], lang, dir)+'</p>'
				}

			// tone correspondences
			if (cols.htone>0 && spreadsheetRows[ch][cols.htone]) {
				out += '<p class="tonePairing">High class equivalent is  '+makeCharacterLink(spreadsheetRows[ch][cols.htone], lang, dir)+'</p>'
				}
			if (cols.ltone>0 && spreadsheetRows[ch][cols.ltone]) {
				out += '<p class="tonePairing">Low class equivalent is '+makeCharacterLink(spreadsheetRows[ch][cols.ltone], lang, dir)+'</p>'
				}
			...
			if (typeof charDetails[ch] === 'undefined') missingDetails += ' '+ch
			else if (charDetails[ch].trim() !== '') out += charDetails[ch]
...
function addSearchEtc () {
    document.getElementById('top').innerHTML = `<img src="../common32/showImages.png" alt="Toggle images" title="Toggle large characters between images and text." onclick="toggleImages()"/><br/><a href="#header-boilerplate"><img src="../../shared/images/up.png" alt="go to page top" title="Jump to top of page."/></a><br><input type="text" id="findInput" placeholder="Find..." style="width: 4em; text-align: center;" onChange="var hex=this.value; if (hex!=''){ document.location = getFindStr(hex); }"/>`
    }   
...
function addDetailsXX (languageName, langClass, lang, dir, spreadsheet, cols) {
    if (traceSet.has('addDetails') || traceSet.has('all')) { console.log('addDetails(',
        'languageName:',languageName,
        'langClass:',langClass,
        'lang:',lang,
    ...
    */
    ...
    for (var x=0; x<noteList.length; x++) {
		...
		if (spreadsheetRows[cchar] && charDetails[cchar]) {
            ...
            out += ` • <a href="../${ orthogFilePath }_terms.html?q=${ cchar }" target="_blank">Term list</a>`
            out += ` • <a href="../../app-charuse/index.html?language=${ charUsageBCP }" target="_blank">Character usage</a>`
            out += `</div>`
...
// <p class="insertTranscription">&#x1A20;</p>
...
function convertTranscriptionData (lang) {
    ...
    console.log(insertTranscriptions.length, cols.othertranscriptions, cols.othertranscriptions.length)
...
function convertTranscriptionData (lang) {
	...
	for (var t=0;t<insertTranscriptions.length;t++) {
		if (insertTranscriptions.length > 0 && cols.othertranscriptions && cols.othertranscriptions.length > 0) {
			var para = ''
			for (var i=0;i<cols.othertranscriptions.length;i++) {
            console.log(insertTranscriptions[t].textContent, cols.othertranscriptions[i][0], spreadsheetRows['ᨠ'][2])
				if (spreadsheetRows[insertTranscriptions[t].textContent] && spreadsheetRows[insertTranscriptions[t].textContent][cols.othertranscriptions[i][0]]) {
                    if (cols.othertranscriptions[i][1]) para += cols.othertranscriptions[i][1]+': '
                    para += '<span class="transc">'+spreadsheetRows[insertTranscriptions[t].textContent][cols.othertranscriptions[i][0]]+'</span>'
					if (i<cols.othertranscriptions.length-1) para += '<br>'
