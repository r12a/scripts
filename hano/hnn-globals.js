

langTag='hnn' // BCP47 language tag for this orthography
scriptTag='hano' // BCP47 script tag for this orthography
notesLangtag='hnn'
blockLangtag = 'hnn' // same as langTag; used in block.html
blockDirection=''  // ltr/rtl or '' for ltr; used by block.html

blockDirectoryName='hano'  // folder name where block file is found, no slash
orthogFilePath='hano/hnn'  // path below /scripts, including filename, but no extension, eg. lana/nod
pickerDir='hano'  // directory name for picker, no slash at end
orthogSummaryTableName='hnn-hano'  // id used for langs record in xx-langdata.js; used to make summary table
scriptSummaryTableName='hano'  // id used for langs record in xx-langdata.js; used to make summary table
charUsageBCP='hnn-hano'  // bcp tag to find the orthography in the charuse app; used in makeIndexIntro only
orthogName = 'Hanunó’o' // natural language name for the orthography; used only in makeIndexIntro
languageName = 'Hanunó’o' // natural language name of language; used in various places, esp block
scriptIsIndic = true  // true if there are indic syllabic categories; used only in makeIndexIntro


defaultSize = '24px'  // used by block.html to set size of font, including in font mgt panel
letterClassName = 'hnn'  // classname for a given orthography in the block page
